...
# 95% довірчий інтервал для середнього рейтингу по жанрах
ci_genres <- cleaned_data %>%
  select(genres, vote_average) %>%
  separate_rows(genres, sep = ",\\s*") %>%
  filter(!is.na(vote_average), !is.na(genres), genres != "") %>%
  group_by(genres) %>%
  summarise(
    mean_rating = mean(vote_average, na.rm = TRUE),
    ...
    n = n(),
    se = sd_rating / sqrt(n),
    lower_ci = mean_rating - qt(0.975, df = n - 1) * se,
    upper_ci = mean_rating + qt(0.975, df = n - 1) * se,
    .groups = "drop"
  )

# Побудова графіка з довірчими інтервалами
ggplot(ci_genres, aes(x = reorder(genres, mean_rating), y = mean_rating)) +
  geom_point(color = "blue", size = 3) +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2, color = "gray40") +
  coord_flip() +
  labs(
    title = "Середній рейтинг за жанрами з 95% довірчими інтервалами",
    x = "Жанр",
    y = "Середній рейтинг"
  ) +
  theme_minimal()
...
# Створення нового стовпця, що містить кількість жанрів для кожного шоу
genre_votes <- genre_votes %>%
  mutate(num_genres = sapply(strsplit(as.character(genres), ",\\s*"), length))

# Тепер ми можемо розглянути зв'язок між кількістю жанрів і середнім рейтингом
# Створюємо новий датафрейм, що містить середній рейтинг для кожної кількості жанрів
genre_avg_filtered <- genre_votes %>%
  group_by(num_genres) %>%
  summarise(
    avg_filtered = mean(vote_average, na.rm = TRUE),
    count = n(),  # Розмір вибірки для кожної кількості жанрів
    sd_filtered = sd(vote_average, na.rm = TRUE),  # Стандартне відхилення
    se_filtered = sd_filtered / sqrt(count),  # Стандартна помилка середнього
    ci_lower = avg_filtered - 1.96 * se_filtered,  # Нижня межа 95% довірчого інтервалу
    ci_upper = avg_filtered + 1.96 * se_filtered,  # Верхня межа 95% довірчого інтервалу
    .groups = "drop"
  )

# Виведемо результат
print(genre_avg_filtered)

# Побудуємо графік з довірчими інтервалами
ggplot(genre_avg_filtered, aes(x = num_genres, y = avg_filtered)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, color = "black") +  # Додаємо довірчі інтервали
  labs(
    title = "Середній рейтинг залежно від кількості жанрів з 95% довірчими інтервалами",
    x = "Кількість жанрІів",
    y = "Середній рейтинг"
  ) +
  theme_minimal()
...
## Множинне тестування
pairwise.t.test(genre_votes$vote_average, genre_votes$num_genres,
                p.adjust.method = "BH")  # або "holm", "BH" — залежно від задачі


# Додаємо до genre_votes колонку з кількістю жанрів
genre_votes <- genre_votes %>%
  mutate(num_genres = sapply(strsplit(as.character(genres), ",\\s*"), length))

# Бутстреп-функція для середнього рейтингу
bootstrap_mean <- function(data, indices) {
  sample_data <- data[indices, ]
  mean(sample_data$vote_average, na.rm = TRUE)
}

# Унікальні кількості жанрів
unique_counts <- sort(unique(genre_votes$num_genres))

# Ініціалізація результату
bootstrap_results <- lapply(unique_counts, function(n) {
  subset_data <- genre_votes %>% filter(num_genres == n, !is.na(vote_average))
  
  # Пропускаємо якщо даних занадто мало
  if (nrow(subset_data) < 10) return(NULL)
  
  boot_result <- boot(data = subset_data, statistic = bootstrap_mean, R = 1000)
  
  tibble(
    num_genres = n,
    mean_bootstrap = mean(boot_result$t),
...
# Об'єднуємо всі результати в один tibble
bootstrap_results <- bind_rows(bootstrap_results)

# Виводимо результат
print(bootstrap_results)

cor.test(genre_votes$num_genres, genre_votes$vote_average)


# Фільтруємо групи з менше ніж 10 фільмів
genre_avg_filtered <- genre_avg_filtered %>%
  filter(count >= 10)

# Побудова оновленого графіка
ggplot(genre_avg_filtered, aes(x = factor(num_genres), y = avg_filtered)) +
  geom_col(fill = "skyblue", color = "black") +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2, color = "black") +
  scale_y_continuous(limits = c(5.8, 7.0), breaks = seq(5.8, 7.0, by = 0.1)) +  # Масштаб Y-осі
  labs(
    title = "Середній рейтинг залежно від кількості жанрів (n ≥ 10)",
    x = "Кількість жанрів",
    y = "Середній рейтинг"
  ) +
  theme_minimal()
